...
{
    public partial class f_YSiBacSi : Form
    {
        OracleConnection con = null;
        public f_YSiBacSi(OracleConnection connect)
        {
            InitializeComponent();
            this.con = connect;
        ...
        {
            OracleCommand cmd = con.CreateCommand();
            cmd.CommandText = "SELECT * FROM QLYCSYT.V_YSIBACSI_HSBA";
            cmd.CommandType = CommandType.Text;
            OracleDataReader dr = cmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
        ...
        {
            OracleCommand cmd = con.CreateCommand();
            cmd.CommandText = "SELECT * FROM QLYCSYT.V_YSIBACSI_HSBA_DV";
            cmd.CommandType = CommandType.Text;
            OracleDataReader dr = cmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
        ...
        private void HienThiBenhNhan(string strFind)
        {
            OracleCommand cmd = con.CreateCommand();
        ...
        private void tabcontrol1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (tabcontrol1.SelectedIndex == 0)
                ...
                this.HienThiBenhNhan(null);
        }

        private void btn_tim_Click(object sender, EventArgs e)
        {
            this.HienThiBenhNhan(txt_tim.Text);
        }
        ...
        private void dgv_hsba_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            OracleCommand cmd = con.CreateCommand();
            cmd.CommandText = "SELECT * FROM QLYCSYT.V_YSIBACSI_HSBA_DV WHERE MAHSBA ='" + this.dgv_hsba.Rows[e.RowIndex].Cells[0].Value.ToString() + "'";
            cmd.CommandType = CommandType.Text;
            OracleDataReader dr = cmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(dr);
